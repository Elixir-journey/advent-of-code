defmodule Mix.Tasks.SetupDayChallenge do
  ...
  defp parse_args([year_str, day_str]) do
  ...
  defp parse_args(_), do: {:error, "Expected 2 arguments: YEAR DAY"}

  @first_year 2015
  @current_year 2025
  defp validate_year(year) when year >= @first_year and year <= @current_year, do: :ok
  defp validate_year(_), do: {:error, "Year must be between #{@first_year} and #{@current_year}"}

  @full_advent_years 2015..2024
  defp validate_day(year, day) do
    cond do
      day < 1 ->
